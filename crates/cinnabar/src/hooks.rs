...
struct CinnabarEyreHook {
	inner: EyreHookFunc,
}

struct CinnabarPanicHook {
	inner: PanicHookFunc,
}

struct CinnabarEyreHander {
	inner: Box<dyn EyreHandler>,
}

impl CinnabarEyreHook {
	fn build_handler(&self, e: &(dyn std::error::Error + 'static)) -> CinnabarEyreHander {
		CinnabarEyreHander { inner: (*self.inner)(e) }
	}
...
impl EyreHandler for CinnabarEyreHander {
	fn debug(
		&self,
...
pub(crate) fn install() -> eyre::Result<()> {
	...
	CinnabarPanicHook { inner: panic_hook.into_panic_hook() }.install();
	CinnabarEyreHook { inner: eyre_hook.into_eyre_hook() }.install()?;
