...
impl AlembicPreprocessor {
		...
		for event in Parser::new(&chapter.content) {
			match event {
				Event::Start(Tag::CodeBlock(CodeBlockKind::Fenced(ref lang))) => {
					if lang.starts_with("alembic") {
						consume_until_closing = true;
						let opts = lang.split(":").collect::<Vec<_>>();

						if opts.len() != 3 {
							eprintln!("Invalid Alembic preprocessor directive: {}", lang);
						} else {
	...
	fn generate_schema(&self, obj: &str, events: &mut Vec<Event>) -> eyre::Result<()> {
		...
		match obj {
			// TODO(aki): Alembic doesn't have config or workspace schema settings for now
			"configuration" => {
				// super::schema::SchemaTomlizer::new(alembic::get_config_schema())
				// 	.render(HeadingLevel::H1, &mut schema_events)?;
				// events.extend(schema_events);
				Ok(())
			},
			"workspace" => {
				// 	super::schema::SchemaTomlizer::new(alembic::get_workspace_schema())
				// 		.render(HeadingLevel::H1, &mut schema_events)?;
				// 	events.extend(schema_events);
				Ok(())
			},
...
impl Preprocessor for AlembicPreprocessor {
	fn name(&self) -> &str {
		"alembic-preprocessor"
	}
