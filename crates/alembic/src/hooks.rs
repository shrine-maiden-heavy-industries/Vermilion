...
struct AlembicEyreHook {
	inner: EyreHookFunc,
}

struct AlembicPanicHook {
	inner: PanicHookFunc,
}

struct AlembicEyreHander {
	inner: Box<dyn EyreHandler>,
}

impl AlembicEyreHook {
	fn build_handler(&self, e: &(dyn std::error::Error + 'static)) -> AlembicEyreHander {
		AlembicEyreHander { inner: (*self.inner)(e) }
	}
...
impl EyreHandler for AlembicEyreHander {
	fn debug(
		&self,
...
pub(crate) fn install() -> eyre::Result<()> {
	...
	AlembicPanicHook { inner: panic_hook.into_panic_hook() }.install();
	AlembicEyreHook { inner: eyre_hook.into_eyre_hook() }.install()?;
