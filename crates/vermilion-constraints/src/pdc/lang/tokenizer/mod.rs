...
impl PdcTokenizer {
	pub fn new(standard: LanguageStd, file: AtomicByteTendril) -> eyre::Result<PdcTokenizer> {
		...
		// Then check to see if it's PDC
		if standard != LanguageStd::Pdc {
			return Err(eyre!(
				"This tokenizer is only supports PDC constraint files"
			));
		}
