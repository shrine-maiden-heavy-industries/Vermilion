...
use crate::{LanguageStd, lpf::lang::tokenizer::token::Token};

pub mod token;

pub struct LpfTokenizer {
...
impl LpfTokenizer {
	pub fn new(standard: LanguageStd, file: AtomicByteTendril) -> eyre::Result<LpfTokenizer> {
		...
		// Then check to see if it's LPF
		if standard != LanguageStd::Lpf {
			return Err(eyre!(
				"This tokenizer is only supports LPF constraint files"
			));
		}
...
impl Iterator for LpfTokenizer {
